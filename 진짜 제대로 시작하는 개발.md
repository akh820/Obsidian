...
- 타이핑 하기전에 내가 뭘치려고했는지 생각하고 치기(자동완성은 켜둬도됨)
...
https://www.erdcloud.com/d/oYPsTAQHLwsxjdB4W

osm - erd 테이블 구조 이미지
![[Pasted image 20260223134359.png]]
...
enum 분리, 그냥 enum패키지 만들면됨
enum을 문자로 사용하고 싶을때는 Enumerated을 사용

###### 2/23
뭘짰는가,
어디서 막혔는가
updatable false 잊지말기
uuid를 entity단에서 만드는거랑 service단에서 만드는거의차이
entity 에서 만들때 장점 -> 바로 uuid를 조회하는 로직이 필요할때
단점 -> 트랜잭션에 실패했을시 메모리낭비(큰문제는아님)

service, 장점 -> 메모리낭비 방지
단점 -> @Setter사용해야함()

jpa의 sql 생성방식?
테이블명 없을시 : OfficeSupply -> office_supply
FK : OfficeSupply(Entity타입) -> office_supply_id(뒤의 id는 baseEntity를 따름)

RESTAPI 컨벤션 
- 케밥케이스
- 리소스는 복수형 /api/supplies

- swagger 메인 페이지 : http://localhost:8080/swagger-ui/index.html
- gradle 의존성 사이트 찾기 쉬운 : https://mvnrepository.com/

기능 뭐뭐 필요하지? 
회원가입, 로그인, 
사용자 : 비품조회, 비품신청(임시), 비품결재신청
결재자(사용자) : 비품결재수락,거절 , 반납확인 
관리자 : 비품등록, 비품수정, 비품삭제, 

DTO를 swagger에서 표기하기 위해서는 ,@Schema 어노테이션 달아줘야함

[[프론트(Front) -> 백(Back) 데이터 전달시 받는 어노테이션(Annotation) 모음]]

@Entity에 @Builder 어노테이션을 달아서 객체를 생성하려고 할시
2개의 어노테이션이 필요하다, 왜 이러한 어노테이션들이 필요한지, 그리고 어떤 이유로
사용되는지 말하여라
@NoArgsContructor : JPA가 리플렉션으로 빈 객체를 생성하기 때문에
(JPA의 내부 동작이 빈 객체 먼저 만들고 나중에 필드 채우는 방식)
@AllArgsContructor : @Builder의 build()가 전체 생성자를 호출 함

Controller에서 상태를 반환할때 2가지 방법이 있는데
둘중 한개만 말해보아라

메서드에 @ResponseStatus(HttpStatus.CREATED), 동적상태코드 불가
반환값에 @ResponseEntity.status(HttpStatus.CREAETD).build();
반환값을 동적으로 변경이  필요할때 사용

@Builder 로 기본값 주입시 주의해야할 점은?
Entity에 @Builder.Default 를 통해서 기본값을 읽어야한다.
없으면 new User() 같은 경우에만 동작
